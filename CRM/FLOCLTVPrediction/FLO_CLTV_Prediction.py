...
data_ = pd.read_csv(r"C:\Users\ibrah\Desktop\miuulBootcamp\pythonProjects\crmAnalytics\FLOCLTVPrediction\flo_data_20k.csv")
data = data_.copy()
data.head()
...
# 1.Veri setindeki en son alışverişin yapıldığı tarihten 2 gün sonrasını analiz tarihi olarak alınız.
data["last_order_date"].max() # 2021-05-30
analysis_date = dt.datetime(2021,6,1)

# 2.customer_id, recency_cltv_weekly, T_weekly, frequency ve monetary_cltv_avg değerlerinin yer aldığı yeni bir cltv dataframe'i oluşturunuz.
cltv_data = pd.DataFrame()
cltv_data["customer_id"] = data["master_id"]

cltv_data["recency_cltv_weekly"] = ((data["last_order_date"]- data["first_order_date"]).dt.days) / 7
cltv_data["T_weekly"] = ((analysis_date - data["first_order_date"]).dt.days) / 7
cltv_data["frequency"] = data["order_num_total"]
cltv_data["monetary_cltv_avg"] = data["customer_value_total"] / data["order_num_total"]
...
bgf.fit(cltv_data['frequency'],
        cltv_data['recency_cltv_weekly'],
        cltv_data['T_weekly'])

# 3 ay içerisinde müşterilerden beklenen satın almaları tahmin ediniz ve exp_sales_3_month olarak cltv dataframe'ine ekleyiniz.
cltv_data["exp_sales_3_month"] = bgf.predict(4*3,
                                       cltv_data['frequency'],
                                       cltv_data['recency_cltv_weekly'],
                                       cltv_data['T_weekly'])

# 6 ay içerisinde müşterilerden beklenen satın almaları tahmin ediniz ve exp_sales_6_month olarak cltv dataframe'ine ekleyiniz.
cltv_data["exp_sales_6_month"] = bgf.predict(4*6,
                                       cltv_data['frequency'],
                                       cltv_data['recency_cltv_weekly'],
                                       cltv_data['T_weekly'])
...
cltv_data.sort_values("exp_sales_3_month",ascending=False)[:10]
cltv_data.sort_values("exp_sales_6_month",ascending=False)[:10]

# 2.  Gamma-Gamma modelini fit ediniz. Müşterilerin ortalama bırakacakları değeri tahminleyip exp_average_value olarak cltv dataframe'ine ekleyiniz.
ggf = GammaGammaFitter(penalizer_coef=0.01)
ggf.fit(cltv_data['frequency'], cltv_data['monetary_cltv_avg'])

cltv_data["exp_average_value"] = ggf.conditional_expected_average_profit(cltv_data['frequency'],
                                                                cltv_data['monetary_cltv_avg'])
...
cltv_data.sort_values("cltv",ascending=False)[:20]
...
cltv_data["cltv_segment"] = pd.qcut(cltv_data["cltv"], 4, labels=["D", "C", "B", "A"])
cltv_data.head()
